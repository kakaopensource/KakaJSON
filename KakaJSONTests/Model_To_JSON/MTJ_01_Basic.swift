...
//        let json = JSONObject(from: car)
        ...
        XCTAssert(int(json?["age"]) == 10)
        XCTAssert(float(json?["area"]) == longFloat)
        XCTAssert(double(json?["weight"])  == longDouble)
        XCTAssert(decimal(json?["height"]) == longDecimal)
        XCTAssert(string(json?["name"]) == "Bently")
        XCTAssert(decimalNumber(json?["price"]) == longDecimalNumber)
        XCTAssert(double(json?["minSpeed"]) == 66.66)
        XCTAssert(number(json?["maxSpeed"]) == 77.77)
